...
images_folder = input_folder / 'fotos_renamed'
...
Inserts renamed images (from extract_images.py output) into the .docx:
  ...
  2. "Marcado_Certificadora.jpg"   → "Reproducción de la etiqueta de la certificadora" cell
  3. All remaining images          → ANEXO 5: FOTOS grid (2 cols × 3 rows per page)
     - Order: General, Marcado_Producto, Fusible, Varistor, Capacitor_X, Bobina,
...
GRID_ORDER = [
    'General', 'Marcado_Producto', 'Fusible', 'Varistor',
    'Capacitor_X', 'Bobina', 'Resistencia', 'Capacitor_Electrolitico',
    'Capacitor_Y', 'Optoacoplador', 'Transformador', 'PCB',
]

SKIP_IN_GRID = {'Marcado_Certificadora', 'Marcado_Producto'}
...
def collect_images(img_folder: Path) -> dict:
    ...
    Scan img_folder and return categorised image paths:
        {
          ...
          'cert':   Path | None,   # Marcado_Certificadora.jpg
    ...
    for p in all_imgs:
        ...
        if 'Marcado_Producto' in stem:
            placa.append(p)
        elif 'Marcado_Certificadora' in stem:
            cert = p
...
def insert_images(docx_path: str | Path,
    ...
    if output_path is None:
        output_path = docx_path.parent / (docx_path.stem + '_images.docx')
    ...
    # ── 2. Insert Marcado_Certificadora into cert cell ────────────────────────
    ...
    if cell_cert is not None and images['cert'] is not None:
        ...
        if images['cert'] is None:
            print("  ⚠ Marcado_Certificadora.jpg not found")
    ...
    for i, ch in enumerate(body_children):
        if ch.tag == f'{W}p':
            ...
            if 'ANEXO 5' in txt and 'FOTOS' in txt:
                anexo5_idx = i; break

    if anexo5_idx is None:
        print("  ⚠ ANEXO 5 heading not found — skipping grid"); 
    else:
        # Find and remove the old photo table immediately after the heading
        old_tbl_idx = None
        for i in range(anexo5_idx + 1, len(body_children)):
            if body_children[i].tag == f'{W}tbl':
                old_tbl_idx = i; break
        ...
        if old_tbl_idx is not None: body.remove(body_children[old_tbl_idx])
        ...
        # Re-attach final_para and sectPr at the end
        if final_para is not None: body.append(final_para)
...
if __name__ == '__main__':
    ...
    # Find input .docx:
    #   - prefer a file with "mod" in the name (already modified in a previous step)
    #   - fall back to any other .docx in the folder
    mod_candidates = [f for f in input_folder.iterdir()
                      if f.is_file()
                      and f.suffix.lower() == '.docx'
                      and 'mod' in f.stem.lower()]

    any_candidates = [f for f in input_folder.iterdir()
                      if f.is_file()
                      and f.suffix.lower() == '.docx']
    ...
    if mod_candidates:
        ...
        print(f"ERROR: No .docx found in: {input_folder}")
        sys.exit(1)

    # Output filename: ensure "mod" is present, then append "_img"
    #   368_C_II.docx              → 368_C_II_mod_img.docx
    #   368_C_II_mod_fields.docx   → 368_C_II_mod_fields_img.docx
    stem = docx_file.stem
    # if 'mod' not in stem.lower():
    #     stem = stem + '_mod'
    output_file = output_folder / (stem + '_img.docx')
    ...
    insert_images(docx_file, images_folder, output_file)

    # Move output back to input_folder with the _img name, delete the original
    final_file = input_folder / output_file.name
    docx_file.unlink()
