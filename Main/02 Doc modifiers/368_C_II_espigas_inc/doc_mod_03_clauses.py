...
def apply_clauses(docx_path:    str | Path,
    ...
    # F.3.3.4 Tensión nominal, F.3.3.4 Frecuencia nominal, F.3.3.6 Rango de
    # corriente o potencia — intentionally left unmodified.
    ...
    for _r in s1_block: _track(_r)

    import copy as _copy

    mod_rows = _mod_rows
    ref_tbl  = find_main_clause_table(root_el)
    ...
    # ── Build rows-only body and save copy (no header / footer) ───────────────
    ...
    if mod_rows:
        ...
        if ref_tbl is not None:
            for tag in (f'{W}tblPr', f'{W}tblGrid'):
                el = ref_tbl.find(tag)
                if el is not None:
                    new_tbl.append(_copy.deepcopy(el))
        for tr in mod_rows:
            new_tbl.append(_copy.deepcopy(tr))
        body.append(new_tbl)

    if sect_pr is not None:
        sp_copy = _copy.deepcopy(sect_pr)
        for tag in (f'{W}headerReference', f'{W}footerReference'):
            for el in sp_copy.findall(tag):
                sp_copy.remove(el)
        body.append(sp_copy)

    copy_dir = Path(output_path).parent / ".docx"
    copy_dir.mkdir(parents=True, exist_ok=True)
    copy_path = copy_dir / "doc_mod_clauses.docx"
    doc_tree.write(str(doc_path), xml_declaration=True, encoding='UTF-8', standalone=True)
    if copy_path.exists():
        copy_path.unlink()
    with zipfile.ZipFile(copy_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        for f in sorted(tmp.rglob('*')):
            if f.is_file():
                zf.write(f, f.relative_to(tmp))
    print(f"  ✔ Clause-rows copy: {copy_path}")
...
if __name__ == '__main__':
    ...
    # Load JSON files
    missing = [f for f in [EUT_basic_json, certificadora_json, componente_json]
               if not (json_folder / f).exists()]
    if missing:
        print(f"ERROR: Missing files in '{json_folder}': {', '.join(missing)}")
        sys.exit(1)

    with open(json_folder / EUT_basic_json,            encoding='utf-8') as f:
        eut_data = json.load(f)
    with open(json_folder / certificadora_json,  encoding='utf-8') as f:
        cert_data = json.load(f)
    with open(json_folder / componente_json,     encoding='utf-8') as f:
        components = json.load(f)

    print(f"\n  File   : {docx_file.name}\n")

    apply_clauses(docx_file, eut_data, cert_data, components)
    print("\nDone.\n")
